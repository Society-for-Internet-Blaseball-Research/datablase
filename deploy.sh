...
(async () => {
  ...
  const versionExists =
    (await exec(`docker images -q ${imageName}`)).stdout.trim().length > 0;

  if (versionExists) {
    console.error(
      `${imageName} already exists! Did you forget to bump the version number?`
    );
    process.exit(1);
  }

  if (!(await execWithOutput(`docker build -t ${imageName} .`)))
    process.exit(1);
  if (!(await execWithOutput(`docker push ${imageName}`))) process.exit(1);
  if (
    !(await execWithOutput(
      `cat k8s/deployment.template.yaml | sed "s/{{IMAGE_VERSION}}/${version}/" | kubectl apply -f -`
    ))
  )
    process.exit(1);
