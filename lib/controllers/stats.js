...
export async function statLeaders({ group, season, type = 'season' } = {}) {
  // Serialize the response of database functions with BigInt returns to either a number or a string
  ...
  for (const statGroup of statGroups) {
    ...
    if (type === 'season') {
      if (statGroup === 'hitting') {
        leaders =
          await prisma.$queryRaw`SELECT * FROM ref_leaderboard_season_batting(${season}::INT)`;
      } else if (statGroup === 'pitching') {
        leaders =
          await prisma.$queryRaw`SELECT * FROM ref_leaderboard_season_pitching(${season}::INT)`;
      } else {
        throw new Error(
