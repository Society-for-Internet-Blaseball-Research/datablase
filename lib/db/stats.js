...
async function countByType(eventType, batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id) FROM game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} event_type=${batterId ? '$2' : '$1'} GROUP BY batter_id`,
...
async function plateAppearances(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id) FROM game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} is_last_event_for_plate_appearance GROUP BY batter_id`,
...
async function atBats(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id) FROM game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} is_last_event_for_plate_appearance AND event_type != 'WALK' AND NOT is_sacrifice_fly GROUP BY batter_id`,
    batterId ? [normalizeCommaSeparatedParam(batterId)] : []
  );
}
...
async function hits(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id) FROM game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} bases_hit > 0 AND event_type != 'WALK' GROUP BY batter_id`,
    batterId ? [normalizeCommaSeparatedParam(batterId)] : []
  );
}
...
async function timesOnBase(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id) FROM game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} bases_hit > 0 GROUP BY batter_id`,
    batterId ? [normalizeCommaSeparatedParam(batterId)] : []
  );
}
...
async function outsRecorded(pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(outs_on_play) AS count FROM game_events ${pitcherId ? 'WHERE pitcher_id = ANY($1)' : ''} GROUP BY pitcher_id`,
...
async function hitsRecorded(pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(id) AS count FROM game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} bases_hit > 0 GROUP BY pitcher_id`,
    pitcherId ? [normalizeCommaSeparatedParam(pitcherId)] : []
  );
}
...
async function walksRecorded(pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(id) AS count FROM game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} event_type = 'WALK' GROUP BY pitcher_id`,
    pitcherId ? [normalizeCommaSeparatedParam(pitcherId)] : []
  );
}
...
async function earnedRuns(pitcherId = undefined) {
  ...
  const [runnersBattedInSet, homeRunsSet] = await Promise.all([
    all(
      `SELECT responsible_pitcher_id, count(id) AS count FROM game_event_base_runners WHERE ${pitcherId ? 'responsible_pitcher_id = ANY($1) AND' : ''} base_after_play = 4 GROUP BY responsible_pitcher_id`,
      pitcherId ? [pitcherList] : []
    ),
    all(
      `SELECT pitcher_id, count(id) AS count FROM game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} event_type='HOME_RUN' GROUP BY pitcher_id`,
      pitcherId ? [pitcherList] : []
    ),
  ]);
