...
async function countBattersByType(eventType, batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id)::int FROM data.game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} event_type=${batterId ? '$2' : '$1'} GROUP BY batter_id`,
...
async function countPitchersByType(eventType, pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(id)::int FROM data.game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} event_type=${pitcherId ? '$2' : '$1'} GROUP BY pitcher_id`,
    pitcherId ? [normalizeCommaSeparatedParam(pitcherId), eventType] : [eventType]
  );
}
...
async function plateAppearances(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id)::int FROM data.game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} is_last_event_for_plate_appearance GROUP BY batter_id`,
...
async function atBats(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id)::int FROM data.game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} is_last_event_for_plate_appearance AND event_type != 'WALK' AND NOT is_sacrifice_fly GROUP BY batter_id`,
    batterId ? [normalizeCommaSeparatedParam(batterId)] : []
  );
}
...
async function hits(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id)::int FROM data.game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} bases_hit > 0 AND event_type != 'WALK' GROUP BY batter_id`,
    batterId ? [normalizeCommaSeparatedParam(batterId)] : []
  );
}
...
async function timesOnBase(batterId = undefined) {
  return await all(
    `SELECT batter_id, count(id)::int FROM data.game_events WHERE ${batterId ? 'batter_id = ANY($1) AND' : ''} bases_hit > 0 GROUP BY batter_id`,
    batterId ? [normalizeCommaSeparatedParam(batterId)] : []
  );
}
...
async function outsRecorded(pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(outs_on_play)::int AS count FROM data.game_events ${pitcherId ? 'WHERE pitcher_id = ANY($1)' : ''} GROUP BY pitcher_id`,
...
async function hitsRecorded(pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(id)::int AS count FROM data.game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} bases_hit > 0 GROUP BY pitcher_id`,
    pitcherId ? [normalizeCommaSeparatedParam(pitcherId)] : []
  );
}
...
async function walksRecorded(pitcherId = undefined) {
  return await all(
    `SELECT pitcher_id, count(id)::int AS count FROM data.game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} event_type = 'WALK' GROUP BY pitcher_id`,
    pitcherId ? [normalizeCommaSeparatedParam(pitcherId)] : []
  );
}
...
async function earnedRuns(pitcherId = undefined) {
  ...
  const [runnersBattedInSet, homeRunsSet] = await Promise.all([
    all(
      `SELECT responsible_pitcher_id, count(id)::int AS count FROM data.game_event_base_runners WHERE ${pitcherId ? 'responsible_pitcher_id = ANY($1) AND' : ''} base_after_play = 4 GROUP BY responsible_pitcher_id`,
      pitcherId ? [pitcherList] : []
    ),
    all(
      `SELECT pitcher_id, count(id)::int AS count FROM data.game_events WHERE ${pitcherId ? 'pitcher_id = ANY($1) AND' : ''} event_type='HOME_RUN' GROUP BY pitcher_id`,
      pitcherId ? [pitcherList] : []
    ),
  ]);
...
async function onBasePercentage(batterId = undefined) {
  ...
  return calculateSet(
    (timesOnBaseValue, plateAppearancesValue) => timesOnBaseValue / plateAppearancesValue,
...
async function slugging(batterId = undefined) {
  ...
  return calculateSet(
    (singlesValue, doublesValue, triplesValue, homeRunsValue, atBatsValue) => {
      return ((singlesValue || 0) + (doublesValue || 0) * 2 + (triplesValue || 0) * 3 + (homeRunsValue || 0) * 4) / atBatsValue;
    },
...
async function whip(pitcherId = undefined) {
  ...
  return calculateSet(
    (outsValue, hitsValue, walksValue) => (hitsValue + walksValue) / (outsValue / 3),
...
async function era(pitcherId = undefined) {
  ...
  return calculateSet(
    (earnedRunsValue, outsRecordedValue) => 27 * (earnedRunsValue / outsRecordedValue),
