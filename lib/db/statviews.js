...
async function seasonBattingStats(season, playerIds) {
  return await seasonStats(
    season,
    playerIds,
    'data.batting_stats_player_season'
  );
}
...
async function seasonPitchingStats(season, playerIds) {
  return await seasonStats(
    season,
    playerIds,
    'data.pitching_stats_player_season'
  );
}
...
async function seasonRunningStats(season, playerIds) {
  return await seasonStats(
    season,
    playerIds,
    'data.running_stats_player_season'
  );
}
...
async function seasonLeaders(season, category, stat, order, limit) {
  ...
  switch (category) {
    case 'batting':
      view = 'data.batting_stats_player_season';
      break;
    case 'pitching':
      view = 'data.pitching_stats_player_season';
      break;
    case 'running':
      view = 'data.running_stats_player_season';
      break;
    case 'fielding':
      view = 'data.fielder_stats_season';
      break;
  }
