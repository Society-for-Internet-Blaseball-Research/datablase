 ...
 */
 router.get('/data/events', async (req, res) => {
	...
	const [gameEvents, baseRunners, playerEvents] = await Promise.all([
    all('SELECT * FROM game_events WHERE season = $1', [season]),
    all('SELECT geb.* FROM game_event_base_runners geb JOIN game_events ge ON geb.game_event_id = ge.id WHERE ge.season = $1', [season]),
    all('SELECT pe.* FROM player_events pe JOIN game_events ge ON pe.game_event_id = ge.id WHERE ge.season = $1', [season]),
  ])
...
router.get('/events', async (req, res) => {
  ...
  const eventResults = await allWithCount(
    `SELECT * FROM game_events WHERE ${filters.join(' AND ')} ${sortBy ? `ORDER BY ${sortBy} ${sortDirection}` : ''}`,
    params
  );
 ...
 * 
 * @route GET /playerAttrs
 * @group Forbidden Knowledge
 * @summary Get the current forbidden knowledge attributes (or 'stlats') for a given player
 * @param {string} playerId.query - The ID of the player
 * @returns {object} 200 - The player's attributes
 */
router.get('/playerAttrs', async (req, res) => {
	const playerId = req.query.playerId;

	if (!playerId) {
		res.json({
		  error: true,
		  message: 'Must specify a playerId.',
		});
		return;
	}
 ...
 * @group Teams
 * @summary Get the current roster for a given team - positions 0 through 8 are the batters, and 9 through 13 are the pitchers
 * @param {string} teamId.query - The ID of the team
 * @returns {object} 200 - array of 'position_id', 'player_id' pairs 
 */
