...
const {
  ...
  seasonRunningStats,
  seasonLeaders,
} = require('../db/statviews.js');
const {
  currentTeams,
  currentTeamStars,
  teamIdBySlug,
} = require('../db/teams.js');
const { sclorigami } = require('../db/sclorigami.js');
...
router.get('/data/events', async (req, res) => {
  ...
  const [gameEvents, baseRunners, outcomes] = await Promise.all([
    ...
    all(
      'SELECT geb.* FROM data.game_event_base_runners geb JOIN data.game_events ge ON geb.game_event_id = ge.id WHERE ge.season = $1',
      [season]
    ),
    all(
      'SELECT pe.* FROM data.outcomes pe JOIN data.game_events ge ON pe.game_event_id = ge.id WHERE ge.season = $1',
      [season]
    ),
  ]);
  ...
  res.setHeader(
    'Content-disposition',
    `attachment; filename=datablase-season${season}.json`
  );
 ...
 * @param {string} batterId.query - The ID of the batter that must be in each event.
 ...
 * @param {enum} sortDirection.query - The direction by which to sort.  - eg: ASC, DESC
...
router.get('/events', async (req, res) => {
  ...
  const includeBaseRunners =
    (req.query.baseRunners || '').toLowerCase() === 'true';
  ...
  if (playerIdFilter) {
    filters = [
      ...filters,
      `(batter_id = ANY($${params.length + 1}) OR pitcher_id = ANY($${
        params.length + 2
      }))`,
    ];
  ...
  const eventResults = await allWithCount(
    `SELECT * FROM data.game_events WHERE ${filters.join(' AND ')} ${
  ...
  if (includeBaseRunners) {
    const baseRunners = await all(
      `SELECT * FROM data.game_event_base_runners WHERE game_event_id = ANY($1)`,
      [ids]
    );

    const baseRunnersById = baseRunners.reduce(
      (acc, baseRunner) => ({
        ...acc,
        [baseRunner.game_event_id]: [
          ...(acc[baseRunner.game_event_id] || []),
          baseRunner,
        ],
      }),
      {}
  ...
  if (includeOutcomes) {
    const outcomes = await all(
      `SELECT * FROM data.outcomes WHERE game_event_id = ANY($1)`,
      [ids]
    );

    const outcomesById = outcomes.reduce(
      (acc, outcome) => ({
        ...acc,
        [outcome.game_event_id]: [
          ...(acc[outcome.game_event_id] || []),
          outcome,
        ],
      }),
      {}
 ...
 *
 * @route GET /v1/floodedRunnersBySeason
 * @group Game Events
 * @summary Calculate the number of events for each batter or pitcher.
 * @param {string} eventType.query.season - The season by which to filter
 * @returns 200 - A JSON file with the raw event data.
 */
router.get('/floodedRunnersBySeason', async (req, res) => {
  const season = req.query.season;

  if (!season) {
    res.json({
      error: true,
      message: 'Must specify season.',
    });
  ...
  const [
    floodEventCount,
    sweptBaseRunnersCount,
    sweptFlippersCount,
  ] = await Promise.all([
    all(
      "SELECT count(id) FROM data.game_events WHERE event_type = 'IMMATERIA_SWEPT' and season = $1",
      [season]
    ),
    all(
      "SELECT count(geb.id) FROM data.game_event_base_runners geb JOIN data.game_events ge ON geb.game_event_id = ge.id WHERE ge.event_type = 'IMMATERIA_SWEPT' and ge.season = $1",
      [season]
    ),
    all(
      `SELECT count(br.id) as swept_runners from data.game_events ge
         JOIN data.game_event_base_runners br on br.game_event_id = ge.id
         JOIN data.outcomes o on ge.id = o.game_event_id
         WHERE ge.event_type = 'IMMATERIA_SWEPT' AND o.event_type = 'SWEPT_FLIPPERS' AND br.runs_scored > 0 and ge.season = $1`,
      [season]
    ),
  ]);
  ...
  res.json({
    flood_event_count: floodEventCount,
    swept_base_runners_count: sweptBaseRunnersCount,
    swept_flippers_count: sweptFlippersCount,
  });
});
...
/**
 * Get data for scorigamis
 *
 * @route GET /v1/sclorigami
 * @group Game Events
 * @summary Get a list of all scorigamis
 * @returns {[object]} 200 - Array of scorigami objects
 */
router.get('/sclorigami', async (req, res) => {
  res.json(await sclorigami());
});
 ...
 * @param {string} name.query - The name of the player (takes precedence if slug is specified)
 * @param {string} slug.query - The url_slug of the player
...
router.get('/playerInfo', async (req, res) => {
  ...
  if (!playerId && !name && !slug) {
    res.json({
      error: true,
      message: 'Must specify a player ID, name, or slug',
    });
  ...
  if (!playerId && !name) {
    // Get the player ID for this slug
    let rawId = await playerIdBySlug(slug);

    if (rawId.length === 0) {
      res.json({
        error: true,
        message: `Player slug ${slug} not found`,
      });

      return;
    }

    playerId = rawId[0].player_id;
  } else if (!playerId) {
 ...
 * @param {enum} position.query - The position desired - eg: BATTER,PITCHER,BENCH,BULLPEN
 * @returns {[RosterEntry]} 200 - array of roster information
 */
router.get('/currentRoster', async (req, res) => {
  let teamId = req.query.teamId;
  let slug = req.query.slug;
  let includeShadows = req.query.includeShadows;
  ...
  res.json(await currentRoster(teamId, includeShadows, position));
});
 ...
 * @summary Get the list of all players as of a specific Season and Gameday
 * @returns {object} 200 - array of player information including current team id/nickname
 */
router.get('/allPlayersForGameday', async (req, res) => {
  const season = req.query.season;
  const day = req.query.day;
  res.json(await allPlayersForGameday(season, day));
});
 ...
 * @param {string} stat.query - Stat to get the leaders for (as returned from /playerStats)
 * @param {string} order.query - Ordering to use for ranking (ASC or DESC)
 * @param {number} limit.query - Number of leaders to return
 * @returns {[object]} 200 - list of leaders
 */
router.get('/seasonLeaders', async (req, res) => {
  ...
  if (
    ...
    res.json({
      error: true,
      message:
        "Must specify a valid stat category ('batting', 'pitching', 'running', 'fielding') as 'category'.",
    });
    return;
  }
  ...
  if (!season || !stat) {
    res.json({
      error: true,
      message: 'Must specify a season and stat',
    });
  }

  res.json(await seasonLeaders(season, category, stat, order, limit));
});
...
router.get('/playerStats', async (req, res) => {
  ...
  if (!playerIds || playerIds.length == 0) {
    res.json({
      error: true,
      message:
        "Must specify a comma-separated list of player IDs as 'playerIds'",
    });
    return;
  }
