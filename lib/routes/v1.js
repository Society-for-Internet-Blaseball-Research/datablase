...
router.get('/floodedRunnersBySeason', async (req, res) => {
  ...
  const [floodEventCount, sweptBaseRunnersCount, sweptFlippersCount] = await Promise.all([
    all('SELECT count(id) FROM data.game_events WHERE event_type = \'IMMATERIA_SWEPT\' and season = $1', [season]),
    all(
    ...
    all('SELECT count(br.id) as swept_runners from data.game_events ge \
