...
router.get('/floodedRunnersBySeason', async (req, res) => {
  ...
  const [gameEvents, baseRunners, outcomes] = await Promise.all([
    all('SELECT * FROM data.game_events WHERE season = $1', [season]),
    all(
      'SELECT geb.* FROM data.game_event_base_runners geb JOIN data.game_events ge ON geb.game_event_id = ge.id WHERE ge.event_type = \'IMMATERIA_SWEPT\' and ge.season = $1',
      [season]
    ),
    all(
      'SELECT pe.* FROM data.outcomes pe JOIN data.game_events ge ON pe.game_event_id = ge.id WHERE ge.season = $1',
