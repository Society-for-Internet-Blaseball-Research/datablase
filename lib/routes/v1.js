 ...
 */
 router.get('/data/events', async (req, res) => {
    ...
    all('SELECT geb.* FROM data.game_event_base_runners geb JOIN game_events ge ON geb.game_event_id = ge.id WHERE ge.season = $1', [season]),
    all('SELECT pe.* FROM data.player_events pe JOIN game_events ge ON pe.game_event_id = ge.id WHERE ge.season = $1', [season]),
  ])
...
router.get('/events', async (req, res) => {
  ...
  if (includePlayerEvents) {
    const playerEvents = await all(`SELECT * FROM data.player_events WHERE game_event_id = ANY($1)`, [ids]);
