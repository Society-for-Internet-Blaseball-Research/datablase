 ...
 * @param {boolean} includeHistory.query - whether to include the players' stlat history
 * @returns {[object]} 200 - list of players
 */
router.get('/allPlayers', async(req, res) => {
	const includeShadows = req.query.includeShadows;
	const includeHistory = req.query.includeHistory;

	res.json(await allPlayers(includeShadows,includeHistory));
});
