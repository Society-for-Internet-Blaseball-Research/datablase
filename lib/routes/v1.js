 ...
 */
 router.get('/data/events', async (req, res) => {
    ...
    all('SELECT * FROM game_event_base_runners JOIN game_events ON game_event_base_runners.game_event_id = game_events.id WHERE game_events.season = $1', [season]),
    all('SELECT * FROM player_events JOIN game_events ON player_events.game_event_id = game_events.id WHERE game_events.season = $1', [season]),
  ])
