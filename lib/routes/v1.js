...
const { deceasedPlayers, playerIdsByName, playerInfoCurrent, playerInfoAll, taggedPlayers, allPlayers, allPlayersForGameday } = require('../db/players.js');
 ...
 * @param {string} playerIds.query - The player ID of the player (takes precedence if a name is also specified)
 * @param {string} name.query - The name of the player
...
router.get('/playerInfo', async (req, res)=>{
	let playerIds = req.query.playerIds;
	let playerId = req.query.playerId;
	const name = req.query.name;
	const all = req.query.all;

	if(!playerIds && !playerId && !name) {
		res.json({
			error: true,
	...
	if(all) {
		res.json(await playerInfoAll(playerIds))
	}
	else
	{
		res.json(await playerInfoCurrent(playerIds))
	}
 ...
 * @param {number} gameday.query - Gameday to query (zero-indexed)
 * @summary Get the list of all players as of a specific Season and Gameday
 * @returns {object} 200 - array of player information including current team id/nickname
 */
router.get('/allPlayersForGameday', async (req, res)=>{	
	const season = req.query.season;
	const gameday = req.query.day;
	res.json(await allPlayersForGameday(season, gameday));
})
