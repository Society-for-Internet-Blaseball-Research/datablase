...
const { deceasedPlayers, playerIdsByName, playerInfoCurrent, playerInfoAll, taggedPlayers } = require('../db/players.js');
 ...
 * @param {string} playerId.query - The player ID of the player
...
router.get('/playerInfo', async (req, res)=>{
	...
	if(!playerId) {
		res.json({
			error: true,
			message: 'Must specify a player ID.',
		});
		return;
	}
