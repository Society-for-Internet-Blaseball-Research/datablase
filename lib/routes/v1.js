...
router.get('/floodedRunnersBySeason', async (req, res) => {
  ...
  const [floodEventCount, sweptBaseRunnersCount, sweptFlippersCount] = await Promise.all([
    ...
    all(
      'SELECT count(geb.id) FROM data.game_event_base_runners geb JOIN data.game_events ge ON geb.game_event_id = ge.id \
      JOIN data.outcomes pe ON pe.game_event_id = ge.id WHERE pe.event_type = \'SWEPT_FLIPPERS\' and ge.season = $1',
      [season]
    )
  ]);
