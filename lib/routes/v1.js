...
const { playerAttrs, deceasedPlayers, playerIdsByName } = require('../db/players.js');
 ...
 */
 router.get('/data/events', async (req, res) => {
    ...
    all('SELECT pe.* FROM data.player_events pe JOIN data.game_events ge ON pe.game_event_id = ge.id WHERE ge.season = $1', [season]),
  ])
 ...
 * @returns {[object]} 200 - array of objects with player IDs matching that name, including the name and a timestamp for when that name stopped being used by that player
 */
