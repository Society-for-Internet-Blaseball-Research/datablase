...
const { seasonBattingStats, lifetimeBattingStats, seasonPitchingStats, lifetimePitchingStats} = require('../db/statviews.js');
...
router.get('/playerStats', async(req, res) => {
	...
	if(!category || (category != 'pitching' && category != 'batting' )) {
		res.json({
			error: true,
			message: "Must specify a valid stat category ('batting' or 'pitching') as 'category'.",
		});
		return;
	}
