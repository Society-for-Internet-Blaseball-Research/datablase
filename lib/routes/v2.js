 ...
 * @group Teams
 * @group APIv2
 * @param {string} teamIdOrSlug.path.required - The team ID or URL slug.
 ...
 * @param {string} skip.query - The number of rows to skip for pagination purposes (e.g. `3`).
 * @param {string} limit.query - The number of rows to return (e.g. `5`).
...
router.get('/players', async (req, res) => {
  ...
  const result = await players({ order, playerPool, season, sortField, skip, limit, fields });
 ...
 * @group Players
 * @group APIv2
 * @param {string} playerIdOrSlug.path.required - The player ID or URL slug.
...
router.get('/stats', async (req, res, next) => {
  ...
  // Return an error when the `season`, `teamId`, `sortStat`, and
